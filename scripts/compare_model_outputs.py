...
def compare_date(date, model_paths, output_dir):
    ...
    for file_key in all_files:
        present_models = [model for model in models if file_key in model_rows[model]]
        ...
        if len(present_models) >= 2:
            ...
            for field in all_fields:
                ...
                for model in present_models:
                    ...
                    normalized.append(normalize_value(field_types[field], raw))
                if missing_any:
        ...
        if diff_fields or missing_models:
            row = {
                "file": file_key,
            ...
            for model in models:
                if model not in present_models:
                    ...
                    continue
                model_field_values = {
                    field: model_values[field][model]
                    for field in diff_fields
                    if model in model_values[field]
                }
    ...
    with open(summary_path, "w", encoding="utf-8", newline="") as handle:
        ...
        for field in all_fields:
            ...
            diff_rate = diff_count / comparable_files if comparable_files else 0
            missing_rate = missing_count / comparable_files if comparable_files else 0
            writer.writerow(
                {
                    "field": field,
