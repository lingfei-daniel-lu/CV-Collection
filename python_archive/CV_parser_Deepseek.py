...
MODEL_NAME  = "deepseek-chat"   # DeepSeek's main chat LLM
TEMPERATURE = 0.0
MAX_RETRIES = 5
...
# DeepSeek uses the OpenAI-compatible API; just change base_url:
client = OpenAI(
...
• "promotion_university": institution where this promotion occurred
• "years_post_phd": years between PhD graduation and promotion
• "journals": object with EXACT journal titles and integer count >=1 or false.

Use null for unknown fields. Do NOT wrap JSON in backticks.
...
def chat_completion(cv_text: str) -> str:
    """Call DeepSeek's Chat Completion API with retries."""
